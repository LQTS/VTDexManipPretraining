...
from vitac_core.conf import TrackingConfig
...
class Metrics:
    def __init__(
        ...
        if self.model_arch in {"v-repic", "t-retac"}:
            self.tracker.update({"reconstruction_loss": deque(maxlen=self.window)})

        elif self.model_arch in {"vtt-reall"}:
            self.tracker.update(
                {
                    "reconstruction_loss": deque(maxlen=self.window),
                    "img_reconstruction_loss": deque(maxlen=self.window),
                    "tac_reconstruction_loss": deque(maxlen=self.window)
                }
            )
        else:
    ...
    def push(self, epoch: int) -> str:
        ...
        elif self.model_arch == "vtt-reall":
            self.log(
                self.global_step,
                metrics={
                    "Pretrain/Step": self.global_step,
                    "Pretrain/Epoch": epoch,
                    "Pretrain/V-ReAll Train Loss": loss,
                    "Pretrain/Reconstruction Loss": itemized["reconstruction_loss"],
                    "Pretrain/Img Reconstruction Loss": itemized["img_reconstruction_loss"],
                    "Pretrain/Tac Reconstruction Loss": itemized["tac_reconstruction_loss"],
                    "Pretrain/Learning Rate": lr,
                    "Pretrain/Step Time": step_time,
                },
            )

        elif self.model_arch in {"t-retac"}:
            self.log(
                self.global_step,
                metrics={
                    "Pretrain/Step": self.global_step,
                    ...
                    "Pretrain/Reconstruction Loss": itemized["reconstruction_loss"],
                    # "Pretrain/Img Reconstruction Loss": itemized["img_reconstruction_loss"],
                    # "Pretrain/Tac Reconstruction Loss": itemized["reconstruction_loss"],
                    "Pretrain/Learning Rate": lr,
                    "Pretrain/Step Time": step_time,
                },
            )
