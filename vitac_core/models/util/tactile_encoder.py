...
from vitac_core.models.util.transformer import get_1D_position_embeddings
from vitac_core.models.pygcn.layers import GraphConvolution
from vitac_core.models.pygcn.graph import Graph
...
class TacFC20_Enc(nn.Module):
    ...
    def forward(self, touch_sensors: torch.Tensor) -> torch.Tensor:
        ...
        tac_embeddings = torch.cat(tac_embeddings_list, dim=-2)
...
class TacFC20_Dec(nn.Module):
    ...
    def forward(self, touch_output: torch.Tensor) -> torch.Tensor:        # (x,20) -> (x, 1, 20)
