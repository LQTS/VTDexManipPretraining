...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None, return_attention=False) -> torch.Tensor:
        a_v, a = self.attn(self.pre_norm_attn(x), mask)
        if return_attention:
            return a
    
        if self.do_layer_scale:
            x = x + self.layer_scale_attn(a_v)
