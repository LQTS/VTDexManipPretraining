...
from vitac_core.models.util.optimization import get_lr_update
from vitac_core.models.util.transformer import Block, PatchEmbed, RMSNorm, get_2D_position_embeddings
from vitac_core.models.util.tactile_encoder import TacConNet
...
class VTT_ReAll(nn.Module):
    def __init__(
        ...
        self.print_shape, self.create_mask_pic = print_shape, create_mask_pic
        self.mask_pic_created = False
        self.lr = None
    ...
    def forward_encoder(
        ...
        if self.print_shape:
            print(f"tac_visible_patches.shape: {tac_visible_patches.shape}")
            print(f"tac_restore_idxs.shape: {tac_restore_idxs.shape}")
            print(f"tac_mask.shape: {tac_mask.shape}")
            print(f"tac_mask: {tac_mask}")
        ...
        if self.print_shape:
            print(f"tac_patches.shape: {tac_patches.shape}")


        # Create "dummy" visible mask, concatenate image patches & language, feed to Transformer
        visible_mask = torch.ones_like(visible_patches[..., -1], dtype=torch.float32)  # (bts, 49) if not cls else (bts, 50)
        tac_visible_mask = torch.ones_like(tac_patches[..., -1], dtype=torch.float32)  # (bts, 10)
        if self.print_shape:
            print(f"tac_visible_mask.shape: {tac_visible_mask.shape}")
        # print(f"tac_visible_mask.shape: {tac_visible_mask.shape}")
        # (bts, 59(60), 384)
        ...
        if tac_visible_mask.shape[-1] == 0:
            visible_patches = multimodal_embedding
            visible_tactile = tac_patches
        else:
    ...
    def forward_tac_decoder(self, tactile_patches: torch.Tensor) -> torch.Tensor:
        ...
        if self.print_shape:
            print(f"tactile_patches.shape: {tactile_patches.size()}")
            print(f"projected_tactile.shape: {projected_tactile.size()}")
        ...
        if self.print_shape:
            print(f"decoder_prediction.shape: {decoder_prediction.shape}")
    ...
    def forward_decoder(
        ...
        # print(f"tac_mask_tokens.shape: {tac_mask_tokens.shape}")
        ...
        # mask pic
        if self.create_mask_pic and not self.mask_pic_created:
            MaskPatches_BlackArea = torch.zeros([concatenated_patches.shape[0], mask_tokens.shape[-2], (self.patch_size**2) * self.in_channels], dtype=torch.float32)
            MaskPatches_WhiteArea = torch.ones([concatenated_patches.shape[0], projected_patches.shape[-2], (self.patch_size**2) * self.in_channels], dtype=torch.float32)
            MaskPatches = torch.cat([MaskPatches_WhiteArea, MaskPatches_BlackArea], dim=1)
            self.mask_patches = torch.gather(
                MaskPatches, dim=1, index=restore_idxs.cpu()[..., None].repeat(1, 1, (self.patch_size**2) * self.in_channels)
            )
            self.mask_pic_created = True
        ...
        # print(f"tac_concatenated_patches.shape: {tac_concatenated_patches.shape}")
        # print(f"tac_unshuffled_patches.shape: {tac_unshuffled_patches.shape}")
    ...
    def generate_origin_img(self, patched_imgs, ori_imgs, mask_patches=None):
        ...
        # recon_patches = patched_imgs

        # depatchify
        recon_imgs = self.depatchify(recon_patches)
        if mask_patches is not None:
            return [recon_imgs, self.depatchify(mask_patches)]
        else:
            return [recon_imgs, None]
    ...
    def compute_tac_loss(self, tactile: torch.Tensor, reconstructions: torch.Tensor):
        ...
        if self.tactile_type in {"binary"}:
            ...
            tactile_loss = avg_loss_per_patch.mean()
        # compute mean loss
    ...
    def compute_loss(self, imgs: torch.Tensor, reconstructions: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        return avg_loss_per_patch.mean()

    def get_last_selfattention(self, img: torch.Tensor, tactile: torch.Tensor):
        ...
        # assert int(self.TacConNet.tac_num_patches * (1- self.tac_mask_ratio)) > 0, \
        #     "tactile mask ratio is too large, visible tac patches is 0 Now!"
        # visible_patches, mask, restore_idxs = self.mask(patches_pe,
        #                                                 mask_ratio)  # (bts, 49, 384), (bts, 196), (bts, 196)
        # tac_visible_patches, tac_mask, tac_restore_idxs = self.mask(tac_patches_pe, self.tac_mask_ratio)
        visible_patches = patches_pe
        tac_visible_patches = tac_patches_pe
        ...
        for idx, block in enumerate(self.encoder_blocks):
            if idx < len(self.encoder_blocks) - 1:
                ...
                attention_map = block(multimodal_embedding, multimodal_mask, return_attention=True)
                # multimodal_attention_map[:, : - tac_visible_mask.shape[-1], ...]

        return attention_map, self.TacConNet.tac_num_patches
    ...
    def forward(
        ...
        if self.tactile_set_to_ZERO:
            mask_idx = torch.randint(img.shape[0], (int(img.shape[0]*0.3),))
            img[mask_idx] = 0
        # forward
        ([visible_patches, mask, restore_idxs],
        ...
        # return loss, [img_reconstructions, tac_reconstructions], [img_loss, tac_loss], self.mask_patches
